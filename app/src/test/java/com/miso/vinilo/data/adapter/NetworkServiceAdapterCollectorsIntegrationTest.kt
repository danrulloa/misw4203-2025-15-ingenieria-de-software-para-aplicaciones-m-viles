...
class NetworkServiceAdapterCollectorsIntegrationTest {
    ...
    @Test
    fun `retrofit parses collector list from mock server`() = runTest {
        val json = """
            [
              {
                "id": 100,
        ...
        val adapter = NetworkServiceAdapterCollectors.create(baseUrl)
        ...
        when (result) {
            is NetworkResult.Success -> {
                ...
                val c = list[0]
                assertEquals(100L, c.id)
                assertEquals("Manolo Bellon", c.name)
                assertEquals("3502457896", c.telephone)
                assertEquals("manollo@caracol.com.co", c.email)
            }
